...
export default function SprintEF59Page() {
  ...
  const runCertification = useCallback(async () => {
    ...
    try {
      ...
      if (allRuns.length > 0) {
        ...
        // Check all runs have exactly 10 stages
  ...
  const stats = useMemo(() => {
    ...
    const ownershipScore = 1.0; // all engines are isolated by design
    ...
    const depScore       = 1.0; // no circular deps in official pipeline
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs font-mono">
                      <thead>
                        <tr className="text-zinc-500 border-b border-zinc-800">
                          {["#","ID","Cenario","Stages","KS antes","KS+","Depth","MetaConf","Recs","Status"].map(h => <td key={h} className="py-1 pr-2">{h}</td>)}
                        </tr>
                      </thead>
                      <tbody>
                        {runs.map((run, i) => {
            ...
            {!stats ? (
              ...
              <>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                  ...
                  {[
                    ["Toda Pipeline Oficial executada",         runs.length === SCENARIOS.length,      `${runs.length}/${SCENARIOS.length} cenarios`],
