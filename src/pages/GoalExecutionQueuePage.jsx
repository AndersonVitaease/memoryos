// Goal Execution Queue v1.0 — Dashboard
// Foundation v1.0 · Engineering First

import React, { useState, useCallback } from "react";
import { runGoalExecutionQueueTests } from "@/lib/goal-execution-queue/goalExecutionQueueTests";
...
export default function GoalExecutionQueuePage() {
  ...
  const handleRun = useCallback(async () => {
    ...
    try { setData(await runGoalExecutionQueueTests()); }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-4 md:p-6">
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-violet-950/60 to-cyan-950/60 border border-violet-800/50 rounded-xl p-5">
          <div className="flex items-start justify-between flex-wrap gap-3">
            <div>
              <div className="flex flex-wrap gap-2 mb-2 text-xs font-mono">
                <span className="text-violet-400">Goal Execution Queue v1.0</span>
                <span className="text-zinc-600">·</span>
                <span className="text-zinc-400">Foundation v1.0</span>
                <span className="text-zinc-600">·</span>
                <span className="text-emerald-400">Engineering First</span>
              </div>
              <h1 className="text-lg font-bold text-white">Goal Execution Order Management</h1>
              <p className="text-zinc-400 text-sm mt-0.5">
                GoalExecutionQueue · Priority Ordering · FIFO · enqueue/dequeue/peek
              </p>
            ...
            <button onClick={handleRun} disabled={running}
              ...
              {running ? "Executando..." : "Executar Queue v1.0"}
            </button>
          </div>
          {data && (
        ...
        {running && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center">
            ...
            <p className="text-zinc-400 text-sm">GoalRuntime → RegistryService → Scheduler → Queue → Priority → FIFO → Hardening...</p>
          ...
          <>
            ...
            <div className={`rounded-xl border-2 p-3 ${allPass ? "bg-emerald-950/30 border-emerald-700" : "bg-red-950/30 border-red-800"}`}>
              <div className="flex items-center gap-3 flex-wrap">
                <Badge
                  label={allPass ? "EXECUTION QUEUE v1.0 — PASS" : "EXECUTION QUEUE v1.0 — FAIL"}
                  style={allPass
                    ? "bg-emerald-900/60 text-emerald-300 border-emerald-700"
                    : "bg-red-900/60 text-red-300 border-red-700"}
                />
                <p className={`text-sm font-bold ${allPass ? "text-emerald-300" : "text-red-300"}`}>
                  {allPass ? "Fila de execucao certificada." : `${data.total - data.passed} criterio(s) reprovado(s).`}
                </p>
              </div>
            ...
            {activeTab === "statistics" && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  <Metric label="Enfileirados" value={data.statistics.enqueued}  color="text-violet-400" />
                  <Metric label="Removidos"    value={data.statistics.removed}   color="text-amber-400" />
                  <Metric label="Processados"  value={data.statistics.processed} color="text-blue-400" />
                  <Metric label="Falhos"       value={data.statistics.failed}    color="text-red-400" />
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <Metric label="Fila Atual" value={data.statistics.queueSize}    color="text-emerald-400" />
                  <Metric label="Fila Max"   value={data.statistics.maxQueue}     color="text-zinc-300" />
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                  ...
                  {["QUEUED","PROCESSING","COMPLETED","FAILED","REMOVED"].map(s => {
                    const count = { QUEUED: data.statistics.queueSize, PROCESSING: 0, COMPLETED: 0, FAILED: data.statistics.failed, REMOVED: data.statistics.removed }[s] ?? 0;
                    return (
                      <div key={s} className="flex items-center gap-3 text-xs mb-1.5">
                        ...
                        <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                          <div className="h-full bg-violet-600 rounded-full"
                            style={{ width: data.statistics.enqueued > 0 ? `${(count / data.statistics.enqueued) * 100}%` : "0%" }} />
                        </div>
            ...
            {activeTab === "metrics" && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  <Metric label="Enqueue"  value={data.metrics.enqueueTotal}  color="text-violet-400" />
                  <Metric label="Dequeue"  value={data.metrics.dequeueTotal}  color="text-blue-400" />
                  <Metric label="Remove"   value={data.metrics.removeTotal}   color="text-amber-400" />
                  <Metric label="Peek"     value={data.metrics.peekTotal}     color="text-sky-400" />
                </div>
                <div className="grid grid-cols-3 gap-2">
            ...
            {activeTab === "arch" && (
              <div className="space-y-3">
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-2">
                  <p className="text-zinc-400 text-xs uppercase tracking-wider mb-3">Componentes — Queue v1.0</p>
                  {[
                    ["GoalExecutionQueue",   "Administra ordem de execucao de Goals"],
                    ["ExecutionQueueEntry",  "Objeto imutavel: queueId, goalId, priority, enqueueTime, status, attempts"],
                    ["Ordering",             "Priority DESC → enqueueTime ASC (FIFO tiebreak)"],
                    ["GoalRuntime v0.1",     "Ciclo de vida — reutilizado"],
                    ["GoalRegistryService",  "Administracao global — reutilizado"],
                    ["GoalScheduler v1.0",   "Agendamento temporal — reutilizado"],
                  ].map(([comp, desc]) => (
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-1">
                  ...
                  {[
                    ...
                    "Apenas: enqueue, dequeue, peek, remove, list, statistics, health, clear",
                  ].map((inv, i) => (
                ...
                <div className="bg-violet-950/20 border border-violet-800 rounded-xl p-4">
                  <p className="text-xs font-bold text-violet-300 mb-2">Entry Status Lifecycle</p>
                  <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
                    {["QUEUED","→","PROCESSING","→","COMPLETED"].map((s, i) => (
                      <span key={i} className={s === "→" ? "text-zinc-600" : "bg-zinc-800 text-violet-300 px-2 py-0.5 rounded border border-violet-900"}>{s}</span>
                    ))}
                  </div>
                  <div className="flex items-center gap-2 text-xs font-mono mt-2">
                    <span className="text-zinc-600">QUEUED/PROCESSING →</span>
                    ...
                    <span className="bg-zinc-800 text-amber-300 px-2 py-0.5 rounded border border-amber-900">REMOVED</span>
                  </div>
                </div>
              </div>
            )}
        ...
        {!data && !running && !error && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center">
            <p className="text-zinc-400 text-sm font-medium mb-1">Goal Execution Queue v1.0 — Execution Order Management</p>
            <p className="text-zinc-600 text-xs">Priority DESC · FIFO tiebreak · enqueue/dequeue/peek/remove</p>
            <p className="text-zinc-700 text-xs mt-1">Reutiliza Goal Runtime + Registry Service + Scheduler</p>
          </div>
        )}
      </div>
    </div>
