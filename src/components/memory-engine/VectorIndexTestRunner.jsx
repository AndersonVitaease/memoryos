...
import { runVectorIndexTests, VECTOR_INDEX_TEST_CASES } from "@/lib/memory-engine";

export default function VectorIndexTestRunner() {
  ...
  const handleRun = async () => {
    ...
    setProgress({});
    try {
      const res = await runVectorIndexTests((p) => {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-zinc-800">
              Memory Vector Index Manager — Indexação Determinística
            </h2>
            <p className="text-sm text-zinc-500 mt-1">
              Indexa embeddings em memória de forma determinística. Não gera embeddings.
              Não realiza buscas. Não modifica Memory Records, Relationships, Lifecycle,
              Version History ou Embeddings. Apenas mantém o índice consistente.
            </p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
            ...
            <ul className="text-xs text-emerald-600 space-y-0.5">
              <li>• addToIndex() — indexar embedding</li>
              <li>• removeFromIndex() — remover do índice</li>
              <li>• rebuildIndex() — reconstruir índice</li>
              <li>• validateIndex() — validar consistência</li>
              <li>• getIndexStats() — estatísticas</li>
              <li>• Rejeita duplicatas, órfãos, dimensões incorretas</li>
            </ul>
          </div>
          <div className="bg-red-50 border border-red-200 rounded-xl p-3">
            ...
            <ul className="text-xs text-red-600 space-y-0.5">
              ...
              <li>• ANN / HNSW / FAISS</li>
              <li>• OpenSearch / Pinecone / Weaviate / Qdrant</li>
              <li>• Modificar embeddings</li>
            </ul>
          </div>
        </div>
      </div>
...
Índice: determinístico em memória
Sem ANN · Sem HNSW · Sem FAISS · Sem banco vetorial`}
        </pre>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-sm font-semibold text-zinc-800">Contrato Memory Vector Index</h2>
        </div>
        <pre className="text-xs bg-zinc-50 rounded-xl p-4 overflow-x-auto text-zinc-600 font-mono">
{`{
  indexId: UUID,
  ...
  status: "active" | "rebuilding" | "corrupted" | "archived"
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            value={auto?.embeddingsIndexed || 0}
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-zinc-800">
            Bateria de Testes ({VECTOR_INDEX_TEST_CASES.length} cenários)
          </h2>
          {summary && (
            <span
        ...
        <div className="space-y-2 mb-4 max-h-[400px] overflow-y-auto">
          {VECTOR_INDEX_TEST_CASES.map((tc) => {
      ...
      {auto && (
        <div className="bg-white border border-zinc-200 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <StatRow label="Embeddings indexados" value={auto.embeddingsIndexed} />
            <StatRow label="Embeddings removidos" value={auto.embeddingsRemoved} />
            <StatRow label="Embeddings rejeitados" value={auto.embeddingsRejected} />
            <StatRow label="Índices reconstruídos" value={auto.indexesRebuilt} />
            <StatRow label="Índices validados" value={auto.indexesValidated} />
            <StatRow label="Tempo médio (ms)" value={auto.averageProcessingTimeMs} />
            <StatRow
              label="Nenhum embedding modificado"
              value={auto.noEmbeddingsModified ? "✓ Confirmado" : "✗ Falhou"}
            />
          </div>
        </div>
      )}
      ...
      {acceptance && (
        <div className="bg-white border border-zinc-200 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <AcceptRow label="Memory Vector Index Manager independente" ok={acceptance.vectorIndexManagerIndependent} />
            <AcceptRow label="Contrato Memory Vector Index existe" ok={acceptance.vectorIndexContractExists} />
            <AcceptRow label="Indexação funcionando" ok={acceptance.indexingWorking} />
            <AcceptRow label="Rebuild funcionando" ok={acceptance.rebuildWorking} />
            <AcceptRow label="Validação funcionando" ok={acceptance.validationWorking} />
            <AcceptRow label="Nenhum Memory Embedding alterado" ok={acceptance.noEmbeddingsModified} />
