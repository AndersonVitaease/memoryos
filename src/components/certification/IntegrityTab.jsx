// IntegrityTab.jsx — Sprint EF-39.6 — SRP: renders integrity checks only
...
export default function IntegrityTab({ report }) {
  const { integrity } = report.auditReport;
  return (
    <div className="border border-zinc-700 rounded-xl bg-zinc-900">
      <div className="px-4 py-3 border-b border-zinc-800 text-xs text-zinc-400 tracking-widest">
        INTEGRITY AUDIT — {integrity.passed}/{integrity.passed + integrity.failed} · {integrity.durationMs}ms
      </div>
      {integrity.checks.map((c, i) => <CheckRow key={i} ok={c.ok} label={c.check} detail={c.detail} />)}
    </div>
  );
}