...
| Sprint | Foco | Entregável | Status |
|---|---|---|---|
| EI-01 | Reversibility Metadata | Campo `reversibility` no metadata de cada connector (`safe` / `reversible` / `irreversible`). Default `safe` quando ausente. Nada le o campo ainda. | Planned |
