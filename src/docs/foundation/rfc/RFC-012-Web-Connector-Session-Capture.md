...
**Validado (2026-08-09, atualizado):** teste ponta a ponta real executado via `/web-connector` contra `https://the-internet.herokuapp.com/login` — fluxo completo `start` → `login` (credenciais `tomsmith`/`SuperSecretPassword!`) → `confirm` → `status=active`, cookies reais capturados e persistidos. Todos os 4 operations (`start`/`login`/`confirm`/`revoke`) validados. **Ainda pendente:** reuso de sessão numa segunda chamada (reinjetar cookies num novo contexto Playwright); expiração automática de TTL (sweep que muda `status` para `expired`).

**Bug corrigido durante a validação (loginVerified false-positivo):** a operação `login` preenchia o formulário via DOM e o submit navegava corretamente para `/secure` (login real funcionando), mas `loginVerified` retornava `false` — impedindo o botão "Confirmar" de aparecer na UI. **Causa raiz:** o `browser_run_code_unsafe` devolvia o `JSON.stringify({...})` de retorno duplamente codificado (string JSON dentro de string JSON), então `JSON.parse` produzia uma string em vez de um objeto, e `loginOutcome.url` ficava `undefined` → `authed` ficava `false`. **Correção:** (1) parse recursivo que detecta `typeof loginOutcome === 'string'` e re-parseia; (2) fallback via snapshot pós-login — se o snapshot contém marcadores positivos de auth (`logout`/`welcome`/`secure area`/`you logged into`) e nenhum campo de senha, marca `authed=true` mesmo se o parse DOM falhar. O snapshot é a fonte de verdade visual quando o JSON do Playwright vem codificado de forma inesperada.

Durante a validação, `browser_run_code_unsafe` (usada na operação `confirm` para capturar cookies via `context.cookies()`, e na operação `login` para preencher o formulário via `ElementHandle`) precisou de 5 iterações para descobrir sua convenção real de chamada (não documentada): o parâmetro `code` deve avaliar para uma **função** `async (page) => { ... }`, que a tool invoca internamente como `__fn__(page)` — statements soltos, blocos, ou expressões com `await` fora de função falham com SyntaxError/TypeError. Documentado em comentário no código (`webConnectorConnect/entry.ts`) para a próxima vez que essa tool for usada.
