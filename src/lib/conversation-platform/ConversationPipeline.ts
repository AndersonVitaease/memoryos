...
class ConversationPipeline {
  ...
  private async _runPipeline(
    ...
    try {
      ...
      // ── [TEMPORARY DIAGNOSTIC] mcp.callTool Producer B runtime trace ────────
      // CHECKPOINT: "Post MCP Producer B Runtime Trace - 2026-08-20"
      // Objetivo: descobrir onde a execucao mcp.callTool falha em Producer B.
      // ESCOPO: dispara SOMENTE para goal.type === "mcp.callTool".
      // Nenhuma logica funcional alterada — observacao pura, fire-and-forget.
      // LIMITACAO: argumentKeys (query/mode/maxResults) sao resolvidos DENTRO do
      // MCPConnector (resolveMcpArguments) e NAO sao visiveis no nivel do pipeline.
      // Aqui so enxergamos step.parameters = {toolName, serverName, rawText, ...}.
      // Nao ha como distinguir "MCPConnector nunca chamado" de "MCPConnector chamado
      // e falhou internamente" sem instrumentar o proprio connector (excluido do escopo).
      if (goalBridgeResult?.goal?.type === "mcp.callTool") {
        try {
          const { base44: _b44diag } = await import("@/api/base44Client");
          const _step = _singleStep;
          const _stepParams = (_step?.parameters as Record<string, unknown>) ?? {};
          const _safeArgKeys = Object.keys(_stepParams).filter(
            (k) => !/token|secret|auth|bearer|key|password|header/i.test(k),
          );
          const _stepResult = executionResult.steps[0] ?? null;
          const _usedRealEngine = _eiOutcome === null;
          const _producerCRan = !hasHighConfidenceCandidate;
          const _bCandidate = candidates.find(
            (c: { source?: string }) => c.source === "connector_runtime" || c.source === "connector",
          );
          const _tracePayload = {
            temporaryDiagnostic: true,
            executionId,
            goalType: goalBridgeResult.goal.type,
            serverName: _stepParams.serverName ?? null,
            toolName: _stepParams.toolName ?? null,
            rawTextLength: typeof _stepParams.rawText === "string" ? _stepParams.rawText.length : null,
            step: {
              connector: _step?.connector ?? null,
              capability: _step?.capability ?? null,
              parameterKeys: _safeArgKeys,
              argumentKeysNote: "query/mode/maxResults resolved inside MCPConnector — not visible at pipeline level",
            },
            eiTried: _singleStep !== null,
            eiOutcomeStatus: _eiOutcome?.status ?? null,
            eiOutcomeMessage: _eiOutcome?.message ? String(_eiOutcome.message).slice(0, 500) : null,
            eiUsable: _eiOutcome?.status === "success",
            usedRealEngine: _usedRealEngine,
            executionResultStatus: executionResult.status,
            executionResultErrors: (executionResult.errors ?? []).slice(0, 3).map((e: string) => String(e).slice(0, 300)),
            stepResultStatus: _stepResult?.status ?? null,
            stepResultOutputKeys:
              _stepResult?.output && typeof _stepResult.output === "object"
                ? Object.keys(_stepResult.output as object).slice(0, 10)
                : null,
            synthesisHandled: synthesis?.handled ?? null,
            synthesisResponseExists: !!synthesis?.response,
            candidateCreatedByProducerB: !!_bCandidate,
            candidateConfidence: _bCandidate?.confidence ?? null,
            hasHighConfidenceCandidate,
            producerCWillRun: _producerCRan,
            selectedSource: arbResult?.selected?.source ?? null,
            selectedConfidence: arbResult?.selected?.confidence ?? null,
            limitation: "Cannot distinguish MCPConnector-never-called vs MCPConnector-called-and-failed without instrumenting MCPConnector (out of scope).",
          };
          void _b44diag.entities.InteractionEvent.create({
            session_id: session.id,
            correlation_id: executionId,
            actor: "system",
            event_type: "mcp_calltool_trace",
            payload: JSON.stringify(_tracePayload),
          }).catch(() => { /* diagnostic — never blocks */ });
        } catch { /* diagnostic — never blocks */ }
      }
