...
const _builtins: GoalDefinition[] = [
  ...
  // matchBySignals() returns the FIRST hit in registration order.
  {
    ...
    extractParams: (msg) => {
      ...
      const stripped = msg
        .replace(/\b(baixar|baixe|baixa|baixo|baixando|download|downloads|exportar|exporte|exporta|abrir|abra|abre|ler|leia|o arquivo|os arquivos|o documento|os documentos|arquivo|arquivos|documento|documentos|por favor|do drive|no drive|drive|de|do|da|para|meu|minha|pc|computador|que)\b/gi, "")
        .replace(/[-–—]/g, " ")
        .replace(/\s+/g, " ")
        .trim();
