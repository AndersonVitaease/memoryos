/**
 * ConnectorKnowledgeProvider.ts
 * Fetches all relevant knowledge for a connector operation.
...
export interface ConnectorKnowledgeBundle {
...
export const ConnectorKnowledgeProvider = Object.freeze({

  fetch(ctx: ConnectorKnowledgeContext): ConnectorKnowledgeBundle {
    const intent = `${ctx.connector} ${ctx.operation} ${ctx.intent}`;
