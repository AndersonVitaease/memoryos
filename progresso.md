...
- **Divergência de SHA resolvida** (docs-only): amend que mudou o SHA descartado via `git reset --hard 1325a44d`; correção reaplicada como commit novo `f2da98e7` → governed merge local-merge AUTO_FF: main = f2da98e7c6d8 (local + checkout /opt/memoryos byte-idênticos, zero merge commit). Zero impacto em produção (docs-only, suiteInputsHash 75df02a93f19 inalterado).
