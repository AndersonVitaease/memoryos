...
const actualToolNames = tools.result.tools.map((tool: { name: string }) => tool.name);
// Basic verification: catalog returned and contains expected core tools
assert.ok(actualToolNames.length > 30, `Expected reasonable number of tools, got ${actualToolNames.length}`);
assert.ok(actualToolNames.includes('engineering.mcp.catalog'), 'Must include catalog tool');
assert.ok(actualToolNames.includes('engineering.code.search'), 'Must include code.search');
assert.ok(actualToolNames.includes('engineering.file.read'), 'Must include file.read');
assert.ok(actualToolNames.includes('engineering.git.status'), 'Must include git.status');
