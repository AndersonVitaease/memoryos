...
test("authenticated MCP endpoint exposes exactly the approved tools", async () => {
  ...
  try {
    ...
    assert.equal(catalog.actualToolCount, 109);
    assert.equal(catalog.catalogVersion, "eng-mcp-tools-v109");
    ...
    assert.equal(access.get("engineering.ocr.read"), "read");
    ...
    assert.equal(access.get("engineering.git.fetch"), "read");
    assert.equal(access.get("engineering.git.merge"), "write");
    assert.equal(access.get("engineering.registry.scope.grant"), "write");
    ...
    assert.equal(access.get("engineering.memory.dedupe.scan"), "read");
    assert.equal(access.get("engineering.memory.merge"), "write");
    ...
    assert.equal(access.get("engineering.security.ids"), "read");
    assert.equal(access.get("engineering.security.scan"), "read");
    ...
    assert.ok(catalogNames.includes("engineering.security.ids"));
    assert.ok(catalogNames.includes("engineering.security.scan"));
