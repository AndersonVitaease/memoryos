...
export const OPERATIONS = Object.freeze(["status", "test", "build", "candidate", "deploy", "smoke", "rollback", "inspect", "restart"]);
const ASYNC = new Set(["deploy", "rollback"]);
const TIMEOUTS = Object.freeze({ status: 30_000, test: 1_200_000, build: 120_000, candidate: 600_000, deploy: 180_000, smoke: 300_000, rollback: 120_000, inspect: 30_000, restart: 30_000 });
...
export function createReleaseRunner(options = {}) {
  const config = { ...DEFAULTS, ...options }; const execute = options.execute ?? ((job) => runPipeline(job, config)); let active = false; let draining = false; let recoveryMarked = 0; let lastRestartId = null; let lastRestartOutcome = null; const startedAt = new Date().toISOString();
