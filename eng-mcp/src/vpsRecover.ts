...
// Scope is hardcoded to ENG-MCP: the ONLY mutable operation is the official release
// runner rollback. The caller can never choose operation, target, applicationId,
// toolName, command, shell, URL, socket, headers, token, image or container: the
// input schema is strictly { execute?, approval? } and the runner operation is a
// fixed constant sent over the official Unix socket channel (callReleaseRunner,
// injected here as runRunner; no new gateway/scheduler/executor/provider exists).
//
...
// evidence in release-state.json (previousContainer/previousImage) and evaluates
// blockers. Reconcile UNKNOWN blocks recovery; reconcile IN_SYNC is BLOCKED
// (NOTHING_TO_RECOVER); only DRIFTED may proceed when the other requirements pass.
// The existing in-flight job signal (deployStatus=IN_PROGRESS, already surfaced by
// runVpsReconcile as DEPLOY_IN_PROGRESS) is reused to refuse incompatible jobs.
//
// Mutation requires execute=true AND approval.approved=true SIMULTANEOUSLY. The
// runner answers 202 {accepted, jobId, status:"queued"} for rollback: 202/queued is
// NEVER RECOVERED. The tool performs bounded official job status polling (same
// jobId/pending/resume mechanics as engineering.release.pipeline); on exhaustion or
// disconnection it returns UNKNOWN/pending with the durable jobId.
//
// Post-validation only runs after a REAL rolled_back job: (1) official runner smoke
// first — the official smoke action re-syncs the release-state production fields
// (productionCatalogHash/toolCount/catalogVersion/productionImageId) that
// rollbackAction leaves referring to the previous release, so without it reconcile
// would compare the rolled-back catalog against the stale expected state; (2) live
// catalog evidence; (3) a fresh reconcile. RECOVERED requires smoke PASS + live
// catalog + reconcile IN_SYNC. Failed validation -> NOT_RECOVERED; insufficient
// evidence -> UNKNOWN. No LLM; no SSH/shell; never writes release-state.json.
...
export const vpsRecoverInputSchema = z.object({
...
export interface VpsRecoverRunnerResponse { httpStatus: number; body: unknown; }
...
async function defaultReadReleaseState(): Promise<unknown> {
...
export async function runVpsRecover(rawInput: unknown, deps: VpsRecoverDeps = {}): Promise<VpsRecoverResult> {
  ...
  try { rawState = await readReleaseState(); } catch { rawState = null; }
  ...
  // 2. Deterministic reuse of engineering.vps.reconcile (container inspection is not
  // injected here, exactly like the engineering.vps.reconcile registration site).
  ...
  } catch { reconcile = { status: "UNKNOWN", findings: [] }; }
  ...
  // 6. Execution: official runner rollback. The operation is a fixed constant; the
  // runner channel (official Unix socket) is injected; nothing caller-controlled
  ...
  // 7. Bounded official job status polling (jobId/pending/resume mechanics reused
  // from the release pipeline; no new mechanism is invented).
  ...
  // re-syncs the release-state production fields that rollbackAction leaves stale),
  // live catalog evidence, then a fresh deterministic reconcile.
  async function finishPostValidation(current: VpsRecoverResult, context: {
    ...
    // 8.1 Official smoke first. This is the official mechanism that re-syncs
    // release-state.json productionCatalogHash/toolCount/catalogVersion/
    // productionImageId with live production after the rollback.
    try {
    ...
    try { catalog = await context.readCatalog(); } catch { catalog = null; }
    validation.catalog = catalog;

    // 8.3 Fresh reconcile against the re-synced expected state.
    let freshState: unknown = null;
    try { freshState = await context.readReleaseState(); } catch { freshState = null; }
    try {
    ...
    } catch { validation.reconcile = "UNKNOWN"; }
