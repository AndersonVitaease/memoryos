...
export function registerEngineeringTools(server: McpServer, repository: RepositoryAdapter, subject: AuthenticatedSubject, repositoryId: string): void {
  ...
  register("engineering.ocr.read", "read", (name) => server.registerTool(name, {
    description: "OCR-01 deterministic LOCAL OCR (READ-ONLY, idempotent, zero LLM): tesseract 5 (LANGS por+eng fixed, OEM 1/PSM 3) behind a versioned deterministic preprocessing pipeline ocr-pre-v2 (Pillow/OpenCV: EXIF transpose, polarity normalization, denoise, CLAHE contrast, scale, OSD 90-degree rotation, projection-profile deskew kept only when it does not lower the page mean confidence; preprocess=false gives the raw baseline). Input is EXACTLY ONE of path (absolute file inside the server OCR inbox /data/ocr-inbox - lexical + realpath containment, O_NOFOLLOW, regular files only) or base64; <=20MB; format by magic bytes: png/jpeg/webp/tiff (multi-frame) and PDF multi-page (poppler pdftoppm @300dpi, maxPages<=30, default 10, truncated flag). Output {text, blocks[{page,text,bbox,confidence}] (granularity block|line|word), lang, orientation, pages[{page,text,blocks,orientation,skewDeg,meanConfidence,wordCount,steps,frame}], pageCount, sourcePageCount, truncated, engine, input{kind,bytes,sha16}, chars}. Structured errors (ERROR-01 envelope): PATH_NOT_FOUND, PATH_DENIED, PATH_INVALID, UNSUPPORTED_FORMAT, OCR_INPUT_TOO_LARGE, OCR_ENGINE_FAILED (corrupt/undecodable images), PDF_RENDER_FAILED, OCR_TIMEOUT - fixed messages only. The extracted text goes ONLY to the caller: audit /data/audit/ocr.jsonl is metadata-only {ts, subjectHash16, pathSha16, inputSha16, chars, pages, duration_ms, error}. Requires bearer scopes engineering:read + engineering:ocr:read (operator-issued).",
