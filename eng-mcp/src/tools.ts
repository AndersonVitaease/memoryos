...
import { ocrReadInputSchema, runOcrRead } from "./ocrRead.ts";
...
export function registerEngineeringTools(server: McpServer, repository: RepositoryAdapter, subject: AuthenticatedSubject, repositoryId: string): void {
  ...
  register("engineering.ocr.read", "read", (name) => server.registerTool(name, {
    description: "OCR-01 deterministic LOCAL OCR (READ-ONLY, idempotent, zero LLM): tesseract 5 (LANGS por+eng fixed, OEM 1/PSM 3) behind a versioned deterministic preprocessing pipeline ocr-pre-v3 (Pillow/OpenCV: EXIF transpose, polarity normalization, denoise, CLAHE contrast, scale, OSD rotation gated at confidence >= 8 - below that the 0/90/180/270 orientations are measured and the highest-confidence one kept (best_of_4), never a blind rotation; projection-profile deskew kept only when it does not lower the page mean confidence; preprocess=false gives the raw baseline). Input is EXACTLY ONE of path (absolute file inside the server OCR inbox /data/ocr-inbox - lexical + realpath containment, O_NOFOLLOW, regular files only) or base64; <=20MB; format by magic bytes: png/jpeg/webp/tiff (multi-frame) and PDF multi-page (poppler pdftoppm @300dpi, maxPages<=30, default 10, truncated flag). Output {text, blocks[{page,text,bbox,confidence}] (granularity block|line|word), lang, orientation, pages[{page,text,blocks,orientation,skewDeg,meanConfidence,wordCount,steps,frame}], pageCount, sourcePageCount, truncated, engine, input{kind,bytes,sha16}, chars}. Structured errors (ERROR-01 envelope): PATH_NOT_FOUND, PATH_DENIED, PATH_INVALID, UNSUPPORTED_FORMAT, OCR_INPUT_TOO_LARGE, OCR_ENGINE_FAILED (corrupt/undecodable images), PDF_RENDER_FAILED, OCR_TIMEOUT - fixed messages only. The extracted text goes ONLY to the caller: audit /data/audit/ocr.jsonl is metadata-only {ts, subjectHash16, pathSha16, inputSha16, chars, pages, duration_ms, error}. Requires bearer scopes engineering:read + engineering:ocr:read (operator-issued).",
    inputSchema: ocrReadInputSchema
  }, async (input) => { requireRead(); requireOcrRead(); return response(await runOcrRead(input, { subject: subject.subject })); }));
  ...
  register("engineering.registry.entry.create", "write", (name) => server.registerTool(name, {
    description: "Governed creation of a token registry entry (PLAN/execute): PLAN previews everything EXCEPT the credential value — subject, scopes, allowedRepositoryIds, expiry, append position, predicted tokenHash16 and the derived credential path; execute requires execute=true + approval.approved=true and appends the entry through the boot-grade validator with an automatic 0600 backup, TOCTOU drift re-check, byte-identical postvalidation and RESTORED on post-failure, then writes the 0600 credential file under /data/credentials/<subject> verifying file-sha16 == registry tokenHash16. Guards: EVERY creation requires an operator-* authorizer (generalized OPERATOR-PAIR-01); an operator-* target additionally requires the authorizer to be an existing operator-* entry (the A/B pair); unknown scopes, existing subjects and reused credentials are refused; the credentialValue enters once and NEVER appears in output or audit (hash16 only). NOTE: the server reads the registry ONCE at boot — the entry authenticates on the next container boot (one engineering.release.pipeline deploy reload), a declared separate step.",
    inputSchema: registryEntryCreateInputSchema
  }, async (input) => { requireRead(); requireWrite(); requireRegistryScopeGrant(); return response(await runRegistryEntryCreate(input, { callerSubject: subject.subject, authorizerHash16: subject.tokenHash16 })); }));
  register("engineering.registry.entry.revoke", "write", (name) => server.registerTool(name, {
    description: "Governed revocation of a token registry entry (PLAN/execute): PLAN previews the exact target (subject, scopes, entry index, planned revokedAt); execute requires execute=true + approval.approved=true and flips ONLY revokedAt (every other field byte-preserved) with the same backup/TOCTOU/postvalidation/RESTORED discipline as the grant, then neutralizes the credential file under /data/credentials/<subject> (regular files only; symlinks are reported and never followed) AFTER the registry mutation is proven. Guards: self-revocation refused by subject AND by bearer hash16 (the authenticating entry of the current call can never revoke itself); an operator-* victim requires an operator-* authorizer distinct from the victim; already-revoked entries are an idempotent NO_OP with zero writes. Authentication dies on the next container boot (one engineering.release.pipeline deploy reload), a declared separate step.",
    inputSchema: registryEntryRevokeInputSchema
  }, async (input) => { requireRead(); requireWrite(); requireRegistryScopeGrant(); return response(await runRegistryEntryRevoke(input, { callerSubject: subject.subject, authorizerHash16: subject.tokenHash16 })); }));
  ...
  register("engineering.mission.preauth", "write", (name) => server.registerTool(name, {
    ...
    requireWrite();
    return response(runMissionPreauth(input, { caller: { subject: subject.subject, scopes: subject.scopes } }));
  }));
