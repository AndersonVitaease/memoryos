import {
  createMcpClientCallTransport as createNeutralMcpClientCallTransport,
  normalizeMcpResult,
  redactSensitive,
  type VpsTransport,
...
// Compatibility surface (extraction): the transport implementation moved verbatim to
// src/vpsTransport.ts (neutral module — no operational endpoints or IDs; the host
// supplies them). vpsDoctor.ts, tools.ts, server.ts and the existing tests import
// these names from vpsChangeSafe.ts, so they are re-exported here unchanged.
export { normalizeMcpResult } from "./vpsTransport.ts";
export type { VpsTransport, VpsTransportCall, VpsTransportResponse } from "./vpsTransport.ts";

// Compatibility wrapper over the neutral factory: preserves the exact pre-extraction
// resolution order (options ?? env ?? operator defaults) for consumers that always
// relied on it (vpsDoctor.ts). Guardian Cloud files import the neutral factory from
// ./vpsTransport.ts instead (no defaults; hosts resolve the values).
export function createMcpClientCallTransport(options: { dokployServerId?: string; endpoint?: string; timeoutMs?: number } = {}): VpsTransport {
  return createNeutralMcpClientCallTransport({
    dokployServerId: options.dokployServerId ?? process.env.ENG_MCP_VPS_DOKPLOY_SERVER_ID ?? DOKPLOY_SERVER_ID_DEFAULT,
    endpoint: options.endpoint ?? process.env.ENG_MCP_AGENT_MEMORY_ENDPOINT ?? DEFAULT_MEMORY_ENDPOINT,
...
export const DEFAULT_MEMORY_ENDPOINT = "https://ever-mind-core.base44.app/functions/agentMemoryBridge";
...
export async function runVpsChangeSafe(subject: string, input: unknown, deps: VpsChangeSafeDeps = {}): Promise<Record<string, unknown>> {
  ...
  const t0 = now();
  const transport = deps.transport ?? createMcpClientCallTransport({
    dokployServerId: deps.dokployServerId ?? process.env.ENG_MCP_VPS_DOKPLOY_SERVER_ID ?? DOKPLOY_SERVER_ID_DEFAULT,
    endpoint: process.env.ENG_MCP_AGENT_MEMORY_ENDPOINT ?? DEFAULT_MEMORY_ENDPOINT,
  });
