...
export const ERROR_FAMILY_RULES: ReadonlyArray<{ match: RegExp; taxonomy: Taxonomy }> = [
  ...
  { match: /^BASE44_/, taxonomy: { category: "provider", retryable: false, remediation: "Base44 SaaS boundary refused or errored; fix the credential/state with the operator, then retry." } },
];
...
export const ERROR_TAXONOMY: Readonly<Record<string, Taxonomy>> = {
  ...
  MEMORY_DEDUPE_RATE_LIMIT: { category: "state", retryable: true, remediation: "The periodic memory dedupe scan (MEMORY-DEDUPE-01) already ran for this project within the last 24h: retry later or use mode 'ondemand' - scans are advisory and rate-limited to bound judge cost." },
  ...
  BASE44_CREDENTIAL_MISSING: { category: "provider", retryable: false, remediation: "The Base44 API key is missing or empty; the operator stages it (vps.secret.write to the credential file), then retry." },
  BASE44_AUTH_REJECTED: { category: "provider", retryable: false, remediation: "Base44 rejected the credential; the operator rotates/fixes the API key, then the call may be retried." },
  BASE44_CLI_FAILED: { category: "provider", retryable: true, remediation: "The Base44 CLI exited non-zero; inspect the recorded cliRun findings, fix the state, then retry." },
  BASE44_CLI_TIMEOUT: { category: "provider", retryable: true, remediation: "The Base44 CLI exceeded its timeout; check connectivity to Base44, then retry with the same governed flow." },
  BASE44_SECRET_NOT_ALLOWED: { category: "validation", retryable: false, remediation: "Only allowlisted secret names can be written; extend the allowlist through reviewed code, then retry." },
  BASE44_APP_ID_REQUIRED: { category: "validation", retryable: false, remediation: "Pass appId explicitly or configure ENG_MCP_BASE44_APP_ID for the Base44 app." },
  BASE44_APP_ID_INVALID: { category: "validation", retryable: false, remediation: "The Base44 app id must be 8-64 chars of [A-Za-z0-9_-] starting alphanumerically; fix and retry." },
  BASE44_FUNCTION_NOT_ALLOWED: { category: "validation", retryable: false, remediation: "Only allowlisted functions can be deployed; extend the allowlist through reviewed code, then retry." },
  BASE44_FUNCTION_NAME_INVALID: { category: "validation", retryable: false, remediation: "Function names must match /^[A-Za-z][A-Za-z0-9_]{0,63}$/; fix the name and retry." },
  BASE44_SOURCE_EMPTY: { category: "validation", retryable: false, remediation: "The secret value source is empty or unset; stage the value or set the env var, then retry." },
  BASE44_SOURCE_NOT_ALLOWED: { category: "validation", retryable: false, remediation: "Staging sources must live under the governed staging prefix; move the file, then retry." },
  BASE44_SOURCE_NOT_FOUND: { category: "validation", retryable: false, remediation: "The staged secret file does not exist; stage it first, then retry." },
  BASE44_SOURCE_NOT_REGULAR_FILE: { category: "validation", retryable: false, remediation: "The staged source must be a regular file (symlinks/devices are refused); fix, then retry." },
  BASE44_SOURCE_PERMS_REFUSED: { category: "validation", retryable: false, remediation: "The staged file must be mode 0600; tighten its permissions, then retry." },
  BASE44_REPO_SOURCE_MISSING: { category: "provider", retryable: false, remediation: "The repo copy of the function source is missing; sync the repository before deploying." },
  BASE44_PULL_LAYOUT_UNKNOWN: { category: "provider", retryable: true, remediation: "The pull did not yield a recognizable function directory; inspect the BASE44_PULL_EVIDENCE finding, then retry." },
  BASE44_PATCH_PATH_NOT_ALLOWED: { category: "validation", retryable: false, remediation: "Patches may only touch files inside the target function's directory; fix the patch path." },
